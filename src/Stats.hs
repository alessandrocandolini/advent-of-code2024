...
import Text.Megaparsec (MonadParsec (try), Parsec, anySingle, manyTill, runParser)
...
renderSuccessOrError _ (Left e) = T.pack (show e)
...
data StatsEnvVariables = StatsConfig
  { session :: String
  , repo :: String
  }
  deriving (Eq, Show)
...
statsConfigParser :: Env.Parser Env.Error StatsEnvVariables
statsConfigParser =
  StatsConfig
    <$> Env.var (Env.str <=< Env.nonempty) "AOC_SESSION" (Env.help "Value of the session cookie, used for login")
    <*> Env.var (Env.str <=< Env.nonempty) "GITHUB_REPOSITORY" (Env.help "Value of the github repository, used to generate the badge path")
...
program' year config = do
  (status, body) <- lift $ fetchPage year (session config)
  if status /= 200
    then throwError (Not200 status)
    else
      (ExceptT . pure . first CannotFindNumberOfStars . parse) body
